...
export default function AdminLoginPage() {
  ...
  const sidebar = <><div><Link to="/" className="drawer-branding flex items-start text-white" onClick={() => setDrawerOpen(false)}><span className="font-display text-base font-bold leading-6 tracking-tight">Amrita Vishwa Vidyapeetham<br /><span className="text-xs font-medium text-orange-300">Nagercoil Campus</span></span></Link><div className="mt-16 drawer-overview"><p className="font-mono text-[10px] uppercase tracking-[0.24em] text-orange-300">Operations / Content desk</p><h2 className="mt-3 font-display text-3xl font-bold">Chapter overview</h2><p className="mt-4 text-sm leading-6 text-slate-500">Sign in to manage events, resources, and the public chapter portal.</p></div></div><Link to="/" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-white" onClick={() => setDrawerOpen(false)}><ArrowLeft className="size-4" /> Public portal</Link></>;
