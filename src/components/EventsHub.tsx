...
import { CalendarDays, ExternalLink, MapPin, MessageCircle, Sparkles } from 'lucide-react';
...
export default function EventsHub() {
  ...
  const visible = useMemo(
    () => filter === 'All' ? events : events.filter((event) => eventStatus(event.event_date) === filter),
    [events, filter],
  );
    ...
    <section className="relative mx-auto max-w-7xl px-6 py-20" id="events">
      <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          ...
          <h2 className="font-display text-4xl font-bold text-white sm:text-5xl">Find your next room.</h2>
          <p className="mt-3 max-w-xl text-slate-400">Workshops, build nights, and conversations for people who like making things.</p>
        </div>
        <div className="flex flex-wrap gap-2" role="tablist" aria-label="Event status">
          {filters.map((item) => (
            <button key={item} type="button" role="tab" aria-selected={filter === item} onClick={() => setFilter(item)} className={`rounded-full border px-4 py-2 text-sm transition active:scale-[0.98] ${filter === item ? 'border-orange-300 bg-orange-300 text-slate-950' : 'border-slate-700 text-slate-400 hover:border-slate-400 hover:text-white'}`}>{item}</button>
          ))}
        </div>
      </div>
      {loading ? (
        <div className="grid gap-4 md:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-60 animate-pulse rounded-2xl border border-slate-800 bg-slate-900/70" />)}</div>
      ) : visible.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-14 text-center"><Sparkles className="mx-auto mb-4 text-orange-300" /><h3 className="font-display text-xl font-semibold text-white">No events in this lane yet.</h3><p className="mt-2 text-slate-400">Check back soon for the next chapter.</p></div>
      ) : (
